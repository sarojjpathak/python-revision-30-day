...
thislist = ["apple", "banana", "cherry"]
mylist = thislist.copy()#after this if we change thislist then mylist will not change because it is a copy of thislist and not a reference to thislist
print(mylist) 
...
#we can also use slicing to copy a list
...
#to join two lists we can use the + operator and that will concatenate the two lists and return a new list.
list1 = ["a", "b", "c"]
list2 = [1, 2, 3]
list3 = list1 + list2
...
# sort()	Sorts the list
# delete()  Removes the list completely not just the items in the list but the list itself